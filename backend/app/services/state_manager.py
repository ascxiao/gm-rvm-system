...
class StateManager:
    ...
    def __init__(self):
        ...
        self.csv_file = Path(__file__).parent.parent.parent / "bottle_logs.csv"
        self._initialize()
        self._init_csv()
    ...
    def _init_csv(self):
        ...
        if not self.csv_file.exists():
            with open(self.csv_file, 'w', newline='') as f:
                writer = csv.writer(f)
                writer.writerow(['timestamp', 'bottle_count', 'detected_class', 'confidence'])
            logger.info(f"CSV log file created at {self.csv_file}")
        else:
            logger.info(f"Using existing CSV log file at {self.csv_file}")

    def _log_to_csv(self, detected_class: str, confidence: float):
        """Append a bottle acceptance record to the CSV file."""
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        try:
            with open(self.csv_file, 'a', newline='') as f:
                writer = csv.writer(f)
                writer.writerow([timestamp, self.bottle_count, detected_class, confidence])
            logger.info(f"Logged bottle #{self.bottle_count} to CSV")
    ...
    def confirm_drop(self) -> SystemState:
        ...
        # Gna log sa csv ang mga bottle lng nga na detect
        self._log_to_csv(
